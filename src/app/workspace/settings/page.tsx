...
export default function WorkspaceSettingsPage() {
  ...
  async function loadWorkspace() {
    ...
    try {
      ...
      setForm({
        name: text(result.data.workspace.name, "Lumio 工作空间"),
      ...
      rightPanel={
        <div className="space-y-4">
          <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-5">
            ...
            <h2 className="text-xl font-black text-white">当前空间</h2>
            <p className="mt-3 break-words text-sm leading-7 text-slate-300">{text(workspace.name, "Lumio 工作空间")}</p>
            <p className="mt-2 break-all text-xs text-slate-500">{text(workspace.slug)}</p>
          </div>
        </div>
      }
    >
          ...
          <div className="max-w-3xl space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">这些信息会影响工作台、通知和后续团队协作默认值。</p>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="grid gap-2 text-sm font-bold text-white">
                ...
                >
                  <option className="bg-slate-950" value="zh-CN">简体中文</option>
                  <option className="bg-slate-950" value="en-US">English</option>
                  <option className="bg-slate-950" value="ja-JP">日本語</option>
                </select>
              </label>
              <label className="grid gap-2 text-sm font-bold text-white">
                默认时区
                ...
                >
                  <option className="bg-slate-950" value="Asia/Shanghai">Asia/Shanghai</option>
                  <option className="bg-slate-950" value="UTC">UTC</option>
                  <option className="bg-slate-950" value="America/Los_Angeles">America/Los_Angeles</option>
                </select>
              </label>
            </div>
